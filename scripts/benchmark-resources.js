/**
 * @file scripts/benchmark-resources.js
 * @description 离线比较资源开销。用法：node scripts/benchmark-resources.js <构建输出目录>。
 */
...
// 保留现有格式化和编码流程，但禁用实际的文件写入。
const writer = localRequire('./LogWriter');
writer.logWriter.enqueue = () => true;
const original = execFileSync('git', ['show', `${baseline}:${loggerPath}`], {
  cwd: root,
  encoding: 'utf8'
});
...
const logSample = (source) => {
  ...
  const sandbox = {
    ...
    require: (name) => {
      ...
      if (name === './LogContext') return {
        getLogScope: () => scope,
        getLogFilePath: () => 'synthetic.txt'
      };
...
const main = async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      baseline,
      probeInvalidConfigChecks: 4,
      probeCreatedRuntimeFiles: false,
      ...
      logs: {
        before: summary(oldLogs),
        after: summary(newLogs),
        samples: {
          before: oldLogs,
          after: newLogs
        }
      },
      probes: Object.fromEntries(Object.entries(probes).map(([entry, samples]) => [entry, {
