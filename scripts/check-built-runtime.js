/**
 * @file scripts/check-built-runtime.js
 * @description 对构建后的 JavaScript 包或 Windows SEA 可执行文件执行本地集成检查。
 */
...
const main = async () => {
  ...
  // Windows 可能分配 fetch 明确禁止使用的临时端口，例如 6665。
  for (;;) {
    try {
  ...
  try {
    ...
    const page = await fetch(base + '/api/logs/dailyQuest/page', {
      headers
    }).then((response) => response.json());
    assert.ok(Buffer.byteLength(page.text) <= 65536 && page.older);
    const earlier = await fetch(base + `/api/logs/dailyQuest/page?cursor=${page.older}`, {
      headers
    }).then((response) => response.json());
    assert.ok(earlier.text && !earlier.reset);
    assert.equal((await fetch(base + '/api/logs/dailyQuest/page?cursor=invalid', {
      headers
    })).status, 400);
    assert.equal(await fetch(base + '/api/logs/dailyQuest', {
      headers
    }).then((response) => response.text()), log);
    assert.equal((await fetch(base + '/api/logs/dailyQuest/download-ticket', {
      method: 'POST',
      headers
    })).status, 404);
    ...
    await fetch(base + '/api/jobs/artifact/stop', {
      method: 'POST',
      headers
    });
    ...
    await fetch(base + '/api/manager/shutdown', {
      method: 'POST',
      headers
    });
    ...
    console.log(JSON.stringify({
      ...
      previewBundled: true,
      onlinePreviewOnly: true,
      nativeDownloadRoutesRemoved: true,
      orderedReplayAndLiveMessages: true,
      gracefulExit: code
    }));
  } finally {
    socket?.terminate();
