/** Offline bounded-resource exercise. node --expose-gc scripts/soak-resources.js [seconds=120] */
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const { monitorEventLoopDelay, performance } = require('node:perf_hooks');
const { LogCache } = require('../dist/tools/logging/LogCache');
const { LogWriter } = require('../dist/tools/logging/LogWriter');
const { setLogSecrets, withLogSecrets, formatLogValue } = require('../dist/tools/logging/sanitize');
...
const sample = () => {
  ...
  return { elapsedMs: now - started, ...process.memoryUsage(), cpuPercentOneCore,
    cacheBytes: cache.byteLength, pendingBytes: writer.pendingBytes, eventLoopP99Ms: delay.percentile(99) / 1e6 };
};
...
const main = async () => {
  ...
  try {
    while (performance.now() - started < seconds * 1000) {
      ...
      setLogSecrets({ password: secret });
      await withLogSecrets({ password: secret }, async () => {
        for (let index = 0; index < 10; index++) {
          ...
          const entry = { id: records++, scope, type: 'log', data };
      ...
      if (performance.now() - started >= nextCheckpoint) {
        ...
        if (global.gc) global.gc();
        checkpoints.push({ elapsedMs: performance.now() - started, ...process.memoryUsage(),
      ...
      if (performance.now() - started >= nextSample) {
        // Spread a bounded number of samples across the entire run, not only its tail.
    ...
    if (global.gc) global.gc();
    console.log(JSON.stringify({ node: process.version, platform: process.platform, seconds, records, batches, readCalls,
      initial, final: sample(), writer: writer.stats, samples, checkpoints, explicitGc: !!global.gc,
      note: 'Synthetic log/cache/secret/read lifecycle only; no real accounts, no production throughput claim.' }, null, 2));
  } finally {
