...
export interface LinkedExtensionsData {
  currentUser?: {
...
export interface ChannelExtensionsData {
  user?: {
  channel?: {
  selfInstalledExtensions?: TwitchInstalledExtension[]
}
}
}
 ...
 */
export const parseLinkedArenaExtension = (payload: Array<TwitchGqlEnvelope<LinkedExtensionsData>>): boolean => !!payload[0]?.data?.currentUser?.linkedExtensions?.some(({
  name
}) => name === 'Arena Rewards Tracker');
