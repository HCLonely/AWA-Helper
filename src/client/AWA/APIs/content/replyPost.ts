 ...
 */
export const replyPost = async (context: AWAContext, requestedPostId?: string): Promise<ActionResult<'replied', 'no-topic' | 'rejected'>> => {
  ...
  if (!postId) return { ok: false, state: 'no-topic' };
