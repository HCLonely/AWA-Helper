...
const reads = new WeakMap<AWAContext, { read: SharedRead<string>; origin: string; revision: number }>();
/**
...
const fetchControlCenter = async (context: AWAContext, signal: AbortSignal): Promise<string> => {
  ...
  const options: myAxiosConfig = {
    url: `${context.baseURL}/control-center`, method: 'GET', signal: deadlineSignal, timeout: 15000, retryTimes: 1, headers: {
      ...context.headers,
...
export const getControlCenter = (context: AWAContext): Promise<string> => {
  ...
  if (!entry || entry.origin !== context.baseURL || entry.revision !== context.readRevision) {
    entry = { read: new SharedRead<string>(), origin: context.baseURL, revision: context.readRevision };
