 ...
 */
export const verifySession = async (context: AWAContext): Promise<{
  userId: string;
  username: string
}> => {
  const options: myAxiosConfig = {
    url: `${context.baseURL}/account`,
    method: 'GET',
  ...
  try {
    ...
    if ($('a.nav-link-login').length) {
      throw new AWAError('verifySession', 'AWA cookie has expired', false, 602);
    }
    ...
    if (!userId || !username) {
      throw new AWAError('verifySession', 'AWA account identity was not found');
    }
    ...
    throw new AWAError('verifySession', 'Unable to verify AWA session', true, undefined, {
      cause: error
    });
  }
};
