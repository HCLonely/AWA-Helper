/**
 * @file src/core/Manager/Diagnostics.ts
 * @description 执行连接诊断，并生成不包含敏感信息的结果。
 */
...
export class Diagnostics {
  ...
  run(config: config, transport?: HttpTransport): Promise<DiagnosticResult[]> {
    ...
    const bounded = transport || createHttpTransport((options) => http({
      ...options,
      signal,
      timeout: 10000,
      retryTimes: 0,
    ...
    this.pending = withLogSecrets(config, () => runWithRequestSignal(signal, async () => {
      const results = await Promise.all([
        probe('awa', async () => {
          ...
          const awa = new AWAContext({
            cookie: config.awaCookie,
            host: config.awaHost,
            proxy: config.proxy,
            userAgent: config.UA,
            transport: bounded
        ...
        probe('twitch', async () => {
          ...
          const twitch = new TwitchContext({
            cookie: config.twitchCookie,
            proxy: config.proxy,
            transport: bounded
          });
          try {
            ...
            const response = await twitch.request<Array<{
              data?: {
              currentUser?: {
              linkedExtensions?: Array<{
              name?: string
            }>
            }
            };
        ...
        probe('asf', async () => {
          ...
          const asf = new ASFContext({
            protocol: config.asfProtocol,
            host: config.asfHost,
            port: config.asfPort,
            password: config.asfPassword,
            botName: config.asfBotname,
            proxy: config.proxy,
            transport: bounded
          });
          try {
            const response = await asf.request<{
              Success?: boolean
            }>({
              url: asf.commandURL.replace(/\/Command$/, '/ASF'),
              method: 'GET'
            });
