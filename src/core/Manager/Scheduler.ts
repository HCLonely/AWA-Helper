...
class Scheduler {
  ...
  private definitions(): Array<{
    ...
    return [
      ...(this.config.dailyQuestCron ? [{
        name: 'dailyQuest',
        job: 'dailyQuest' as const,
        cron: this.config.dailyQuestCron
      }] : []),
      ...(this.config.achievement.enable ? [{
        name: 'achievement',
        job: 'achievement' as const,
        cron: this.config.achievement.cron
      }] : []),
      ...this.config.artifacts.map((item, index) => ({
        ...item,
        name: `artifact#${index + 1}`,
        job: 'artifact' as const
      }))
    ];
  }
  ...
  static occurrences(expression: string, timezone: string, now: Date, count: number): string[] {
    ...
    const matcher = cron.createTask(expression, () => {}, {
      timezone
    });
    try {
      let iterator = CronExpressionParser.parse(expression, {
        currentDate: now,
        tz: timezone
      });
      const dates: string[] = [];
      for (let attempts = 0; attempts < 36600 && dates.length < count; attempts++) {
        ...
        // 跳过不匹配日期的剩余时间，避免逐秒扫描。
        const midnight = CronExpressionParser.parse('0 0 0 * * *', {
          currentDate: candidate,
          tz: timezone
        });
        const boundary = new Date(midnight.next().toDate().getTime() - 1);
        iterator = CronExpressionParser.parse(expression, {
  ...
  list() {
    return this.definitions().map((item) => ({
      ...
      active: this.started,
      nextRuns: Scheduler.preview(item.cron, this.config.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone)
    }));
  }
