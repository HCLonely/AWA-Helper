---
layout: home

hero:
  name: AWA-Helper
  text: Alienware Arena Task Helper
  tagline: Manage daily quests, achievement quests, and artifact schedules in one place
  actions:
    - theme: brand
      text: Get Started
      link: /en/guide/getting-started
    - theme: alt
      text: Configuration Reference
      link: /en/reference/configuration

features:
  - title: Centralized Management
    details: Manage DailyQuest, Achievement, and Artifact tasks through AWA-Manager and its WebUI.
  - title: Multiple Deployment Options
    details: Run AWA-Helper on Windows, Linux, Node.js, or Docker.
  - title: Automated Tasks
    details: Schedule Alienware Arena online-time, Twitch, Steam, and other tasks.
---

## Get Started

If this is your first time using AWA-Helper, read the [Getting Started](/en/guide/getting-started) guide to choose an installation method and link your accounts. Then use the [Configuration Reference](/en/reference/configuration) to configure cookies, task types, and schedules.

::: warning Steam safety
Running Steam games in the background may carry an anti-cheat risk. Stop AWA-Helper before playing games with anti-cheat systems, such as Call of Duty.
:::
