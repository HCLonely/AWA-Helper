/**
 * @file tests/client-api.test.js
 * @description 在请求层验证可注入的平台传输实现。
 */
...
const {
  getControlCenter
} = require('../dist/client/AWA/APIs/quests');
const {
  TwitchContext
} = require('../dist/client/Twitch/TwitchContext');
const {
  getChannelInfo
} = require('../dist/client/Twitch/APIs/channels');
const {
  ASFContext
} = require('../dist/client/Steam/ASFContext');
const {
  executeCommand
} = require('../dist/client/Steam/APIs/commands');
const {
  playGames
} = require('../dist/client/Steam/APIs/bot');
const {
  BattlePassAPI
} = require('../dist/client/AWA/APIs/battlePass');
...
test('platform HTTP request logging is disabled by default and requires explicit opt-in', () => {
  ...
  assert.equal(new AWAContext({
    cookie: '',
    transport
  }).logRequests, false);
  assert.equal(new TwitchContext({
    cookie: '',
    transport
  }).logRequests, false);
  assert.equal(new ASFContext({
    protocol: 'http',
    host: 'localhost',
    port: 1242,
    botName: 'bot',
...
test('AWA APIs use injected transport and persist response cookies in context', async () => {
  ...
  const transport = {
    request: async (config) => {
      request = config;
      return response('<html>control center</html>', {
...
test('Twitch GraphQL API uses injected transport and platform-only endpoint', async () => {
  ...
  const context = new TwitchContext({
    cookie: 'auth-token=token; unique_id=device',
    userAgent: 'test-agent',
    transport
  });
  context.clientId = 'client-id';
  assert.deepEqual(await getChannelInfo(context, 'streamer'), {
    found: true,
    value: '42'
  });
...
test('AWA control center renews an incomplete login session and keeps redirect cookies', async () => {
  ...
  const context = new AWAContext({
    cookie: 'REMEMBERME=remember;PHPSESSID=stale;sc=stale;home_site=arena.example',
    host: 'arena.example',
    transport: {
      request: async (config) => {
        if (++calls === 1) {
        ...
        config.beforeRedirect(redirect, {
          headers: {
            'set-cookie': ['PHPSESSID=fresh; Path=/', 'sc=fresh; Path=/']
          },
          statusCode: 302
        });
...
test('AWA control center recovery is bounded and requires a missing login record plus REMEMBERME', async () => {
  for (const [cookie, html, expected] of [
    ...
    const context = new AWAContext({
      cookie,
      transport: {
        request: async () => { calls++; return response(html); }
      }
    });
  ...
  const context = new AWAContext({
    ...
    transport: {
      request: async (config) => {
        ...
        assert.throws(() => config.beforeRedirect(redirect, {
          headers: {
...
test('ASF applies the configured proxy agent for its target protocol', async () => {
  ...
  const httpContext = new ASFContext({
    protocol: 'http',
    host: 'asf.local',
    port: 1242,
    botName: 'bot',
  ...
  const httpsContext = new ASFContext({
    protocol: 'https',
    host: 'asf.local',
    port: 1242,
    botName: 'bot',
...
test('remote operations return discriminated business results', async () => {
  ...
  assert.deepEqual(await playGames(context, []), {
    ok: false,
    state: 'no-games'
  });
...
test('Battle Pass claim rejects cross-origin paths and mismatched milestones', async () => {
  ...
  const api = new BattlePassAPI(new AWAContext({
    cookie: '',
    host: 'arena.example',
    transport
  }));
  const reward = {
    ...
    state: 'unlockable',
    claim: {
      path: 'https://evil.example/claim',
      csrfToken: 'token'
    }
  };
  assert.deepEqual(await api.claim('https://arena.example/control-center/battle-pass/1', reward), {
    ok: false,
