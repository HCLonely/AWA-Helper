/** Fixture-style tests for AWA HTML parsers kept outside HTTP APIs. */
...
test('Steam quest parsers return structured list, detail, and progress data', () => {
  ...
  assert.deepEqual(parseSteamQuestListings(list, 'https://arena.example'), [{
    name: 'demo', link: 'https://arena.example/steam/quests/demo', time: 2, arp: 15
  }]);
  assert.deepEqual(parseSteamQuestDetail('<img src="https://cdn/steam/apps/123/header.jpg">Launch Game'), { appId: '123', state: 'ready' });
...
test('AWA feature parsers do not require a client singleton', () => {
  ...
  assert.deepEqual(parseAvailableStreams(streams), { Hive: ['hive_user'], Nexus: [] });
