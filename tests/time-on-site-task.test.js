...
const fixture = (cancelDuringCooldown = false) => {
  ...
  let requests = 0;
  const runtime = {
    state: { trackError: 0, trackTimes: 0, questInfo: { timeOnSite: { addedArp: '0', maxArp: '1' } } },
  ...
  return { waits, runtime, requests: () => requests, run: () => sandbox.exports.TimeOnSiteTask.do(runtime, controller.signal) };
};
...
test('online heartbeat waits five minutes after each six failures and resumes until complete', async () => {
  ...
  assert.deepEqual(task.waits.filter(({ seconds }) => seconds === 300), [
    { seconds: 300, requests: 6 },
    { seconds: 300, requests: 12 }
  ]);
  assert.equal(task.waits.length, 13);
  assert.ok(task.waits.every(({ seconds, requests }) => seconds === (requests % 6 === 0 ? 300 : 60)));
});
