...
test('browser preserves collapsed settings, supports empty artifacts and bounds live logs', { skip: !browser, timeout: 60000 }, async (t) => {
  ...
  const server = http.createServer((req, res) => { res.setHeader('Content-Type', 'text/html; charset=utf-8'); res.end(page(req.url.slice(1))); });
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  for (const mode of ['collapsed', 'empty', 'delete-last', 'logs']) {
