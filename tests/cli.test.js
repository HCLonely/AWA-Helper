...
test('no CLI arguments start persistent Manager mode', () => {
  assert.deepEqual(parseArguments([]), { kind: 'run', mode: 'persistent', deprecatedHelper: false, trayChild: false });
});

test('--daily and legacy --helper select one-shot Manager mode', () => {
  assert.deepEqual(parseArguments(['--daily']), { kind: 'run', mode: 'once', deprecatedHelper: false, trayChild: false });
  assert.deepEqual(parseArguments(['--helper']), { kind: 'run', mode: 'once', deprecatedHelper: true, trayChild: false });
});

test('internal tray child mode keeps persistent Manager semantics', () => {
  assert.deepEqual(parseArguments(['--manager', '--tray-child']), {
    kind: 'run', mode: 'persistent', deprecatedHelper: false, trayChild: true
  });
