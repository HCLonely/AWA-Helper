/**
 * @file tests/twitch-heartbeat-retry.test.js
 * @description 验证 Twitch 心跳失败后的重试行为。
 */
...
const {
  AWAError
} = require('../dist/client/AWA/AWAError');
...
const fixture = (outcomes, cancelAtCooldown = false) => {
  ...
  return {
    ...
    run: () => task.run(controller.signal)
  };
};
...
test('Twitch heartbeat retries each six network failures after five minutes, then recovers', async () => {
  const task = fixture(Array.from({
    length: 12
  }, networkError));
  assert.equal(await task.run(), true);
  assert.equal(task.requests(), 13);
  assert.deepEqual(task.waits, Array.from({
    length: 12
  }, (_, index) => ({
    seconds: (index + 1) % 6 === 0 ? 300 : 60,
    requests: index + 1
  })));
});
...
test('Twitch heartbeat retains one authorization retry for repeated 403 responses', async () => {
  const task = fixture(Array.from({
    length: 2
  }, () => new AWAError('request', 'forbidden', false, 403)));
...
test('Twitch heartbeat still ends on permanent errors and unsuccessful business responses', async () => {
  for (const outcome of [new AWAError('request', 'unauthorized', false, 401), {
    success: false,
    state: 'unknown'
  }]) {
