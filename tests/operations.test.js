...
const { RunHistory, withRunHistory, trackRunStep } = require('../dist/core/Manager/RunHistory');
const { Scheduler } = require('../dist/core/Manager/Scheduler');
const { JobCoordinator } = require('../dist/core/Manager/JobCoordinator');
const { Diagnostics, classifyDiagnosticError } = require('../dist/core/Manager/Diagnostics');
const { PageParseError, parseVerifiedControlCenter } = require('../dist/client/AWA/parsers/verifiedControlCenter');
const { withLogSecrets } = require('../dist/tools/logging/sanitize');
const { validateHelperConfig } = require('../dist/tools/config/ConfigSchema');
...
const config = extra => ({ secret: 'synthetic-manager-secret', timezone: 'UTC', achievement: { enable: false }, artifacts: [], ...extra });
...
test('history survives restart, marks unfinished runs interrupted and leaves finished records intact', async t => {
  ...
  first.step(interrupted, { name: 'Twitch', status: 'running', startedAt: new Date().toISOString() });
...
test('history bounds completed runs while keeping active runs and records sanitized step failures', async t => {
  ...
  await withLogSecrets({ awaCookie: 'REMEMBERME=synthetic-sensitive-value' }, async () => {
...
test('malformed nested history is quarantined instead of crashing startup', t => {
  const file = path.join(directory(t), 'history.json');
  fs.writeFileSync(file, JSON.stringify({ version: 1, runs: [{ id: 'old', name: 'dailyQuest', source: 'schedule', status: 'running', startedAt: new Date().toISOString(), steps: [null] }] }));
...
test('coordinator records one run for duplicate requests, the trigger source, child results and cancellation', async () => {
  ...
  coordinator.register({ name: 'dailyQuest', run: signal => trackRunStep('wait', async () => {
    ready.resolve(); await new Promise(resolve => signal.addEventListener('abort', resolve, { once: true })); return false;
  }) });
...
test('manual stop invalidates a pending scheduled request', async t => {
  ...
  coordinator.register({ name: 'dailyQuest', run: signal => new Promise(resolve => {
    runs++; signal.addEventListener('abort', () => resolve(false), { once: true });
  }) });
...
test('diagnostic probes respect Retry-After beyond the normal result cache', async () => {
  const diagnostics = new Diagnostics(); const raw = { ...defaultConfig, awaCookie: 'REMEMBERME=synthetic-rate-limit' };
  let calls = 0;
  const transport = { request: async () => { calls++; throw { response: { status: 429, headers: { 'retry-after': '120' } } }; } };
...
test('scheduler startup and reload wait for future triggers without starting interrupted jobs', async t => {
  const coordinator = new JobCoordinator(); let runs = 0;
  coordinator.register({ name: 'dailyQuest', run: async () => { runs++; return true; } });
  const id = coordinator.history.begin('dailyQuest', 'schedule'); coordinator.history.finish(id, 'interrupted');
  const settings = config({ dailyQuestCron: '0 0 29 2 *' });
...
test('a reloaded completed daily quest does not claim its reward again', async () => {
  let claims = 0;
  const task = new DailyTask({ state: { questInfo: { dailyQuest: [{ id: '42', status: 'complete' }] } }, claimQuest: async () => { claims++; } });
...
test('diagnostics classify wrapped 429 responses, login failures and parser metadata without returning credentials', () => {
  const result = classifyDiagnosticError('awa', new Error('synthetic-secret', { cause: { response: { status: 429, headers: { 'retry-after': '120', cookie: 'secret' } } } }));
  assert.equal(result.code, 'rate-limited'); assert.ok(Date.parse(result.retryAt) > Date.now() + 119000);
  assert.equal(JSON.stringify(result).includes('secret'), false);
  assert.equal(classifyDiagnosticError('asf', { response: { status: 401 } }).code, 'session-expired');
...
test('connection diagnostics use read-only probes, validate Twitch authorization, and coalesce duplicate requests', async () => {
  ...
  const transport = { request: async options => {
    requests.push(options);
    if (options.url.includes('control-center')) return { data: sample('empty'), status: 200, headers: {} };
    if (options.url === 'https://www.twitch.tv/') return { data: '<script>clientId="fixtureclientid"</script>', status: 200, headers: {} };
    if (options.url.includes('gql.twitch')) return { data: [{ data: { currentUser: { linkedExtensions: [] } } }], status: 200, headers: {} };
    return { data: { Success: true }, status: 200, headers: {} };
  } };
  const raw = { ...defaultConfig, awaCookie: 'REMEMBERME=synthetic-awa', awaQuests: ['watchTwitch', 'steamQuest'],
    twitchCookie: 'auth-token=synthetic-token; unique_id=synthetic-id', asfHost: 'localhost', asfPort: 1242, asfBotname: 'fixture' };
  ...
  await diagnostics.run({ ...raw, awaCookie: '' }, transport); assert.equal(diagnostics.snapshot()[0].code, 'missing-config');
});

test('new configuration fields validate and preserve compatibility defaults', () => {
  const normalized = normalizeManagerConfig({ ...defaultConfig, manager: {} });
  assert.equal(normalized.historyLimit, 200);
  const raw = { ...defaultConfig, manager: { timezone: 'Invalid/Zone', historyLimit: 0 } };
...
test('operations API requires authentication, validates previews and exports parseable redacted JSON', async t => {
  ...
  }, Buffer, process, setInterval, clearInterval, setTimeout, clearTimeout, setImmediate,
  __: key => key, globalThis: global, structuredClone };
  ...
  const { port } = free.address(); await new Promise(resolve => free.close(resolve));
  ...
  setLogSecrets({ manager: { secret: 'synthetic-export-secret' } });
  const coordinator = new JobCoordinator(); coordinator.register({ name: 'dailyQuest', run: async () => true });
  await coordinator.start('dailyQuest');
  const settings = config({ dailyQuestCron: '0 14 * * *' });
  const loaded = { path: 'unused.yml', manager: settings, raw: { ...defaultConfig, awaCookie: '', webUI: { enable: true, local: true, port } } };
  global.wsClients = new Set();
  const scheduler = new Scheduler(coordinator, settings);
  const server = new sandbox.exports.UnifiedServer(loaded, coordinator, 'fixture-version', () => {}, () => ({ restartRequired: false }), scheduler);
  await server.start();
  try {
    ...
    assert.equal((await fetch(`${origin}/api/diagnostics`, { method: 'POST' })).status, 401);
    const headers = { Authorization: `Bearer ${settings.secret}`, 'Content-Type': 'application/json' };
    const history = await (await fetch(`${origin}/api/history`, { headers })).json(); assert.equal(history.runs.length, 1);
    const preview = await fetch(`${origin}/api/schedules/preview`, { method: 'POST', headers, body: JSON.stringify({ cron: 'bad', timezone: 'UTC' }) });
    assert.equal(preview.status, 400);
    const checks = await (await fetch(`${origin}/api/diagnostics`, { method: 'POST', headers, body: '{}' })).json();
    assert.equal(checks.checks[0].code, 'missing-config');
    const exported = await fetch(`${origin}/api/diagnostics/export`, { headers });
    ...
    assert.equal((await fetch(`${origin}/api/diagnostics`, { method: 'POST', headers, body: '{}' })).status, 503);
...
for (const scenario of ['cached', 'missing-output', 'download-failure']) {
  test(`Windows icon packaging fails closed and reuses its local tool: ${scenario}`, async () => {
    ...
    const dependencies = {
      fs: fakeFs, path, 'stream/promises': {},
      axios: { get: async () => { downloads++; throw new Error('Download unavailable'); } },
      child_process: { execFileSync: () => { output = scenario === 'cached'; } }
    };
    const sandbox = { module, exports: module.exports, require: name => dependencies[name], console: { log() {}, error() {} } };
