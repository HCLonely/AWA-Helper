/**
 * @file tests/helpers/browser.js
 * @description 提供浏览器页面加载、执行及等待测试结果的辅助工具。
 */
...
/** 使用真实时钟等待明确的页面结果，同时覆盖 requestAnimationFrame 的执行。 */
async function runBrowser(url, {
  executable = findBrowser(), timeout = 20000
} = {}) {
  ...
  try {
    ...
    const disconnect = (error) => {
      ...
      for (const {
        reject
      } of pending.values()) reject(error);
      pending.clear();
    };
    ...
    while (Date.now() < deadline) {
      const result = await send('Runtime.evaluate', {
        expression: 'document.querySelector("#test-result")?.textContent ?? null',
        returnByValue: true
      }, sessionId);
    ...
    if (child) {
      if (send && child.exitCode === null) {
        try { await send('Browser.close', {}, undefined, 2000); } catch { /* 浏览器可能在确认关闭前断开连接。 */ }
      }
