/**
 * @file tests/daily-runner-lifecycle.test.js
 * @description 验证每日任务运行器的启动、结束与资源释放。
 */
...
const fixture = ({
  cancelDuringSetup = false, failSetup = false, failPush = false, failCommit = false, questError
} = {}) => {
  ...
  const dependencies = {
    ...
    './DailyQuestRuntime': {
      DailyQuestRuntime: Runtime
    },
    './QuestReporter': {
      formatQuestReport: () => ({})
    },
    './QuestFailure': require('../dist/core/DailyQuest/QuestFailure'),
    './tasks/TimeOnSiteTask': {
      TimeOnSiteTask: {
        do: async (_runtime, signal) => {
    ...
    '../../tools': {
      Logger,
      time: () => '',
      configureWebUiColors: () => {},
      checkUpdate: async () => {},
      pushQuestInfoFormat: () => '',
      push: async (message) => { events.push(`push: ${message}`); if (failPush) throw new Error('push failed'); },
    ...
    '../../tools/config': {
      loadConfig: () => ({
        ...
        raw: {
          language: 'en',
          awaCookie: 'old',
          awaQuests: ['timeOnSite', ...(failSetup ? ['watchTwitch'] : [])],
          webUI: {
            enable: false
          }
        }
    ...
    '../../tools/config/YamlConfig': {
      createCookieCommit: () => () => {
        commits++; events.push('cookie committed');
        if (failCommit && commits > 1) throw new Error('disk write failed');
      }
    },
    '../../tools/http/RequestContext': {
      runWithRequestSignal
    },
    '../../tools/logging/sanitize': {
      setLogSecrets: () => {}
    },
    '../../tools/logging/retention': {
      cleanupExpiredLogs: () => {}
    },
    '../../client/shared': {
      DEFAULT_AWA_HOST: 'www.alienwarearena.com',
  ...
  dependencies['../../tools/config/RunConfiguration'] = {
    hasRunConfiguration: () => false,
    getRunConfiguration: dependencies['../../tools/config'].loadConfig,
    createSessionCommit: dependencies['../../tools/config/YamlConfig'].createCookieCommit
  };
  const sandbox = {
    exports: {},
    require: (name) => dependencies[name] || (name === 'chalk' ? require('chalk') : {}),
    globalThis: {},
    __: (key) => key,
    process: {
      argv: [],
  ...
  return {
    ...
    controller,
    releaseChild,
    run: () => sandbox.exports.runDailyQuest({
...
test('DailyQuest logs rejected task reasons immediately and includes them in the final push', async () => {
  const run = fixture({
    questError: new Error('connection refused')
  });
